...
# x的梯度 tensor([4., 3.]),这说明什么问题呢???k是x的函数,k中有两个函数,,要确定这两个函数的值,k.backward是k对变量求导,但是实际上k由x决定,而x里面有两个元素,k的x求导,k包含两个元素,x里面也包含两个元素,那么这种情况该怎么办呢???首先k对x求导,包括k[0]对x求导和k[1]对x求导,首先讨论第一种情况,k[0]对x求导,其实是k[0]对x[0]求导与k[0]对x[1]求导的和,  然后是k[1]对x求导,是k[1]对x[0]求导与k[1]对x[1]求导的和.....
print("x的梯度",x.grad)

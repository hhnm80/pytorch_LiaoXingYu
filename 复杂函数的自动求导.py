...
# ones_like这个函数名字有点意思,如果只是ones函数,就是生成的张量(矩阵)里面的数据都是1,但是我们知道数据都是1了并没有用,因为还不知道张量的形状,这个like就是像的意思,ones_like函数的意义就是创建一个与某个矩阵结构一样,但是里面元素值全是1的新的矩阵,在这里,就是创建一个与n形状相同的矩阵,里面的元素全为1...作为参数传递给backward函数....
